...
args = [
    f"/I\"'{vars['SDK_INCLUDE_PATH']}/um'\"",
    f"/I\"'{vars['SDK_INCLUDE_PATH']}/ucrt'\"",
    f"/I\"'{vars['SDK_INCLUDE_PATH']}/shared'\"",
    f"/I\"'{vars['SDK_INCLUDE_PATH']}/winrt'\"",
    f"/I\"'{vars['SDK_INCLUDE_PATH']}/cppwinrt'\"",
    f"/I\"{vars['MSVC_PATH']}/atlmfc/include\"",
    f"/I\"{vars['MSVC_PATH']}/include\"",
    f"/I\"{vars['VS_PATH']}/VC/Auxiliary/VS/include\"",
    "/EHsc",
    f"/Fe:\"{cur_dir}{exe_path}\"",
    f"/{vars['CXX_VERSION']}",
    "/GA", 
    "/MT",
    "${file}",
    "/link",
    f"/LIBPATH:\"'{vars['SDK_LIB_PATH']}/um/x64'\"",
    f"/LIBPATH:\"'{vars['SDK_LIB_PATH']}/ucrt/x64'\"",
    f"/LIBPATH:\"{vars['MSVC_PATH']}/atlmfc/lib/x64\"",
    f"/LIBPATH:\"{vars['MSVC_PATH']}/lib/x64\"",
    f"/LIBPATH:\"{vars['VS_PATH']}/VC/Auxiliary/VS/lib/x64\"",
]

def main():
    tasks['tasks'][0]['command'] = vars["MSVC_PATH"] + "/bin/Hostx64/x64/cl.exe"
    tasks['tasks'][0]['args'] = args
    launch['configurations'][0]['program'] = cur_dir + exe_path
